...
extract_types(const TemporaryProject &project) {
    ...
    if (!extracted.has_value()) {
        ...
        diagnostic =
            extracted.error().message + " :: " + failed_translation_unit;
    }
    INFO(diagnostic);
    ...
    REQUIRE(analysis.dependencies ==
            std::vector<archscope::clang_backend::ExtractedDependency>{
                {"src/alpha.cpp", (project.root() / "src/alpha.cpp").string(),
                 "sample_app", "<global>", "Derived", "src/beta.cpp",
                 (project.root() / "src/beta.cpp").string(), "sample_domain",
                 "<global>", "Beta", false},
                {"src/alpha.cpp", (project.root() / "src/alpha.cpp").string(),
                 "sample_app", "<global>", "Alpha", "src/beta.cpp",
                 (project.root() / "src/beta.cpp").string(), "sample_domain",
                 "<global>", "Beta", false},
                {"src/alpha.cpp", (project.root() / "src/alpha.cpp").string(),
                 "sample_app", "<global>", "Alpha", "src/beta.cpp",
...
TEST_CASE("clang tool runner excludes free-function dependencies from class "
          "metadata",
          "[clang][extract]") {
...
TEST_CASE("clang tool runner reports analysis failures", "[clang][extract]") {
    ...
    REQUIRE(extracted.error().message.find(
                "failed to parse translation unit") != std::string::npos);
}
...
TEST_CASE("clang tool runner records dependency definition paths for header "
    ...
    TemporaryProject project(
        "archscope-clang-tool-runner-header-dependencies");
    ...
    REQUIRE(analysis.dependencies ==
            std::vector<archscope::clang_backend::ExtractedDependency>{
                {"src/alpha.cpp", (project.root() / "src/alpha.cpp").string(),
                 "sample_app", "sample", "sample::SourceDefined", "",
                 (project.root() / "include/shared.hpp").string(), "",
                 "sample", "sample::HeaderOnly", false},
            });
}
