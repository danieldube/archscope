...
- Prefer local scripts for verification: `./scripts/configure.sh`, `./scripts/build.sh`, `./scripts/test.sh`, `pre-commit run --all-files`, `./scripts/run-tidy.sh`.
- Formatting is enforced through `pre-commit`; C/C++ style comes from `.clang-format`.
