...
26. Do not launch two `cmake --build` commands against the same build directory in parallel. This session triggered a transient `ar` bus error while linking `libarchscope_core.a`; serialize builds per build tree and only parallelize independent read-only inspections or commands that use separate output directories.
