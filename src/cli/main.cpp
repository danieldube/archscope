...
int run_cli(const std::vector<std::string> &args) {
    ...
    const auto metric_registry =
        archscope::core::MetricRegistry::with_defaults();
    ...
    for (const std::string &path : database.value().translation_unit_paths()) {
        archscope::core::ReportModule module{
            path,
            metric_registry.compute(analysis_result,
                                    archscope::core::ModuleId{path},
                                    parsed->metrics),
        };
