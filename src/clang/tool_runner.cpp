    ...
    for (std::size_t index = 0; index < normalized.size(); ++index) {
        ...
        if (argument == "-I" || argument == "-isystem" ||
            ...
            if (index + 1U < normalized.size()) {
                normalized[index + 1U] =
                    resolve_path(entry, normalized[index + 1U]);
                ++index;
            }
            continue;
        }
...
to_compile_command(const core::CompilationDatabaseEntry &entry) {
    return clang::tooling::CompileCommand(
        entry.working_directory, resolve_path(entry, entry.source_path),
...
make_dependency(const SourceManager &source_manager,
                ...
                const std::string &from_qualified_type,
    ...
    const std::string target_qualified_type =
...
void collect_dependencies_from_type(
    ...
    if (const auto *pointer_type = type->getAs<clang::PointerType>()) {
        collect_dependencies_from_type(
            ...
            from_compilation_target, from_namespace_module,
            from_qualified_type, dependencies);
        return;
    }
...
class AnalysisCollector
    : public clang::RecursiveASTVisitor<AnalysisCollector> {
  public:
    ...
    bool VisitCXXRecordDecl(CXXRecordDecl *record) {
        ...
        for (const clang::CXXBaseSpecifier &base : record->bases()) {
            collect_dependencies_from_type(
                ...
                namespace_module, qualified_name, result_.dependencies);
        }
    ...
    bool VisitFunctionDecl(FunctionDecl *function) {
        ...
        // Policy: class dependency graph only. Dependencies from free
        // functions (including namespace-scope functions) are excluded to
        // avoid assigning synthetic class ownership and to keep edge emission
        // deterministic.
        if (function->getDeclContext() == nullptr ||
            !function->getDeclContext()->isRecord()) {
            return true;
        }

        const auto spelling_location =
            context_.getSourceManager().getSpellingLoc(
        ...
        if (from_qualified_type.empty()) {
            return true;
        }

        const std::string definition_path = normalize_path(file_name.str());
        const std::string namespace_module =
            resolve_namespace_module(*function);
        collect_dependencies_from_type(
            function->getReturnType(), context_, translation_unit_paths_,
            translation_unit_path_, definition_path, compilation_target_,
            namespace_module, from_qualified_type, result_.dependencies);
  ...
  public:
    ...
    CreateASTConsumer(clang::CompilerInstance &, llvm::StringRef) override {
        return std::make_unique<AnalysisCollectingConsumer>(
            translation_unit_path_, compilation_target_,
            translation_unit_paths_, result_);
    }
...
Result<ExtractionResult> extract_translation_unit(
    const core::CompilationDatabaseEntry &entry,
    const TranslationUnitPathMap &translation_unit_paths) {
    ExtractionResult result;
    ...
    std::sort(
        ...
        [](const ExtractedType &left, const ExtractedType &right) {
            return std::tie(left.translation_unit_path, left.definition_path,
                   ...
                   std::tie(right.translation_unit_path, right.definition_path,
                            right.compilation_target, right.qualified_name);
        });
}
